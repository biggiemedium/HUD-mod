...
public abstract class MixinGuiNewChat extends Gui {
    ...
    private void translate(CallbackInfo ci) {
        float y = 0;
        if(HUDMod.elementInitalizer.isElementToggled(ChatModifications.class) && HUDMod.elementInitalizer.getElementByClass(ChatModifications.class).fade.getValue() && !this.isScrolled) {
    ...
    private int modifyTextOpacity(int original) {
        if(HUDMod.elementInitalizer.isElementToggled(ChatModifications.class) && HUDMod.elementInitalizer.getElementByClass(ChatModifications.class).fade.getValue() && lineBeingDrawn <= newLines) {
