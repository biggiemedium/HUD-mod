...
public class AltManagerGUI extends GuiScreen {
    ...
    private final Renderutil.ScissorStack scissorStack = new Renderutil.ScissorStack();
    ...
    public void initGui() {
        ...
        if (!HUDMod.altManager.getAltEntries().isEmpty()) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        // Title
        drawCenteredString(mc.fontRendererObj, "Cosmos Alt Manager", scaledResolution.getScaledWidth() / 2, 10, 0xFFFFFF);
        ...
        // Draw alt entries
        HUDMod.altManager.getAltEntries().forEach(entry -> {
        ...
        if (HUDMod.altManager.getAltEntries().isEmpty()) {
            Fontutil.drawTextShadow("No alts!", (int) (scaledResolution.getScaledWidth() / 2f) - Fontutil.getWidth("No alts!") / 2F + 0.75F, (int) 50 - (int) Fontutil.getHeight() / 2F + 2F, 0xFFFFFF);
        }

        // 'Currently logged in as' text
        Fontutil.drawTextShadow("Currently logged in as " + ChatFormatting.GRAY + mc.getSession().getUsername(), 3, 3, 0xFFFFFF);
        ...
        switch (button.id) {
            ...
            case 1:
                ...
                if (getSelectedAltEntry() != null) {
                    // Remove the entry
                    HUDMod.altManager.getAltEntries().remove(getSelectedAltEntry());
    ...
    private void refreshOffsets() {
        ...
        for (AltEntry altEntry : HUDMod.altManager.getAltEntries()) {
    ...
    private AltEntry getFirstAlt() {
        return HUDMod.altManager.indexOf(0);
    }
    ...
    private AltEntry getLastAlt() {
        return HUDMod.altManager.indexOf(HUDMod.altManager.getAltEntries().size() - 1);
    }
