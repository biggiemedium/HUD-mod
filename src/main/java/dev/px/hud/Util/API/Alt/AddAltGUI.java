...
public class AddAltGUI extends GuiScreen {
    ...
    public void initGui() {
        // Email and password
        loginField = new GuiTextField(1, mc.fontRendererObj, width / 2 - 100, height / 2 - 42, 200, 15);
        passwordField = new GuiTextField(2, mc.fontRendererObj, width / 2 - 100, height / 2 - 20, 200, 15);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        // Title
        drawCenteredString(mc.fontRendererObj, "Add Alt Account", scaledResolution.getScaledWidth() / 2, 10, 0xFFFFFF);

        // Draw email text box
        loginField.drawTextBox();

        if (loginField.getText().isEmpty() && !loginField.isFocused() && loginField.getVisible()) {
            Fontutil.drawTextShadow(ChatFormatting.GRAY + "Login", loginField.xPosition + 3, loginField.yPosition + 3.5f, -1);
        }

        // Draw password text box
        passwordField.drawTextBox();
        if (passwordField.getText().isEmpty() && !passwordField.isFocused() && passwordField.getVisible()) {
            Fontutil.drawTextShadow(ChatFormatting.GRAY + "Password", passwordField.xPosition + 3, passwordField.yPosition + 3.5f, -1);
        }
        ...
        switch (button.id) {
            case 3:
                ...
                if (!(loginField.getText().isEmpty())) {
                    // Add alt
                    HUDMod.altManager.getAltEntries().add(new AltEntry(new Alt(loginField.getText(), passwordField.getText(), currentType), AltManagerGUI.altEntryOffset));
                    // Increase offset
